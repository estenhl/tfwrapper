...
class NeuralNet(SupervisedModel):
	...
	def loss_function(self):
		return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(self.pred, self.y, name='softmax'), name='reduce_mean')

	def optimizer_function(self):
		return tf.train.AdamOptimizer(learning_rate=self.learning_rate, name='adam').minimize(self.loss)

	def accuracy_function(self, correct_pred):
		return tf.reduce_mean(tf.cast(correct_pred, tf.float32))

	def fullyconnected(self, prev, weight, bias, name=None):
		fc = tf.reshape(prev, [-1, weight.get_shape().as_list()[0]])

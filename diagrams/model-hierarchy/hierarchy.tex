...
\begin{tikzpicture}[scale=1.5]
    ...
    \node (predictive) [below right=0.3 and 1.8cm of basemodel, draw, dashed, fill=lightblue, rounded rectangle] {Predictive};
    \node (fixedclassification) [below left=2cm and -0.25cm of basemodel, draw, dashed, fill=lightblue, rounded rectangle] {FixedClassificationModel};
    \node (fixedregression) [below right=2cm and -0.25cm of basemodel, draw, dashed, fill=lightblue, rounded rectangle] {FixedRegressionModel};
    \node (trainable) [below=2cm of predictive, draw, dashed, fill=lightblue, rounded rectangle] {Trainable};
    \node (classification) [below=2cm of fixedclassification, draw, dashed, fill=lightblue, rounded rectangle] {ClassificationModel};
    \node (regression) [below=2cm of fixedregression, draw, dashed, fill=lightblue, rounded rectangle] {RegressionModel};
    \node (derivable) [left=of classification, draw, dashed, fill=lightblue, rounded rectangle] {Derivable};

    \node (nn) [below right=3cm and 0.5cm of classification, draw, densely dotted, fill=blue, rounded rectangle] {NeuralNet};
    ...
    \node (metamodel) [below right=13.5cm and 0cm of basemodel, draw, dashed, fill=yellow, rounded rectangle] {MetaModel};
    ...
    \draw[->]
    (basemodel) edge (fixedclassification)
    (basemodel) edge (fixedregression)
    (predictive) edge (fixedclassification)
    (predictive) edge (fixedregression)
    (fixedclassification) edge (classification)
    (fixedregression) edge (regression)
    (classification) edge (nn)
    (trainable) edge (classification)
    (trainable) edge (regression)
    (derivable) edge (nn)
    (frozen) edge (finc3)
    (frozen) edge (finc4)
    (frozen) edge (fres50)
    ...
    (cnn) edge (squeeze)
    (cnn) edge (unet)
    (regression) edge (linear)

    (linear) edge (svm)
    (linear) edge (linreg)
    (fixedclassification) edge (frozen)

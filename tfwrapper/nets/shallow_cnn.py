...
class ShallowCNN(CNN):
	...
	def layers(self, X_shape, name):
		...
		layers = [
			...
			lambda x: self.conv2d(x, self.weight(weights['conv2'], name=self.name + '_wc2'), self.bias(biases['conv3'], name=self.name + '_bc2'), name=name + '_conv2'),
			lambda x: self.conv2d(x, self.weight(weights['conv3'], name=self.name + '_wc3'), self.bias(biases['conv4'], name=self.name + '_bc3'), name=name + '_conv3'),
